...
class ScheduleExport {
	...
	public static function parseDataToScheduleExport() {
		...
		if(empty($schedule_optional_name)){
			...
			$check_for_existing_schedule = $wpdb->get_results("SELECT id FROM {$wpdb->prefix}ultimate_csv_importer_export_template WHERE file_name = '$schedule_file_name' AND module = '$schedule_module_name' AND optionalType = '$schedule_optional_name' ");
