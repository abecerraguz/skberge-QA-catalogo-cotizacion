	...
	public function schedule_import($data_array){
		...
		if($file_extension == 'xml'){
			...
			for($line_number = 0; $line_number < $total_rows ; $line_number++){
				...
				$get_arr = ScheduleImport::$save_mapping_instance->main_import_process($mapping , $header_array['header'] ,$value_array['value'] , $selected_type , $get_mode, $line_number , $check , $hash_key, '');
				$post_id = $get_arr['id'];	
				$core_instance->detailed_log = $get_arr['detail_log'];
