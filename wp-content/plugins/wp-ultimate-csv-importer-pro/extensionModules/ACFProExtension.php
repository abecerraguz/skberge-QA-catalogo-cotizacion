				...
				if ( ! empty( $get_acf_flexible_content_fields ) ) {
					foreach ( $get_acf_flexible_content_fields as $acf_pro_fc_fields ) {
						...
						if($get_sub_field_content['type'] !== 'flexible_content'){
							$customFields["RF"][ $acf_pro_fc_fields->post_name ]['label'] = $acf_pro_fc_fields->post_title;
							$customFields["RF"][ $acf_pro_fc_fields->post_name ]['name']  = $acf_pro_fc_fields->post_excerpt;
						}
						...
						if($get_sub_field_content['type'] == 'flexible_content'){
							...
							foreach($inner_flexi_query as $inner_flexi_values){
								$customFields["RF"][ $inner_flexi_values->post_name ]['label'] = $inner_flexi_values->post_title;
								$customFields["RF"][ $inner_flexi_values->post_name ]['name']  = $inner_flexi_values->post_excerpt;
							}
						}
					}
					...
					foreach($flexible_fields as $flexible_id){
						...
						foreach($get_flexible_group as $get_flexible_value){
							$customFields["RF"][$get_flexible_value->post_name]['label'] = $get_flexible_value->post_title;
							$customFields["RF"][$get_flexible_value->post_name]['name']  = $get_flexible_value->post_excerpt;
						}	
					}
				}
			}
