...
class SaveMapping
{
			...
			if (($h = fopen($upload_dir . $hash_key . '/' . $hash_key, "r")) !== FALSE) {
				...
				while (($data = fgetcsv($h, 0, $delimiters[$array_index])) !== FALSE) {
					...
					if ($i >= $line_number && $i <= $limit) {
						$value_array = $info[$i];
						$get_arr = $this->main_import_process($map, $header_array, $value_array, $selected_type, $get_mode, $i, $check, $hash_key, $update_based_on);
						$post_id = $get_arr['id'];
		...
		if ($file_extension == 'xml') {
			...
			for ($line_number = 0; $line_number < $total_rows; $line_number++) {
				if ( $i >= $lined_number && $i <= $limit) {
					...
					$get_arr = $this->main_import_process($mapping, $header_array['header'], $value_array['value'], $selected_type, $get_mode, $i, $check, $hash_key, $update_based_on);
					$post_id = $get_arr['id'];
					$core_instance->detailed_log = $get_arr['detail_log'];
	...
	public function main_import_process($map, $header_array, $value_array, $selected_type, $get_mode, $line_number, $check, $hash_key, $update_based_on )
	{
		$return_arr = [];
		$core_instance = CoreFieldsImport::getInstance();
		...
		foreach ($map as $group_name => $group_value) {
			if ($group_name == 'CORE') {
				$core_instance = CoreFieldsImport::getInstance();
				$post_id = $core_instance->set_core_values($header_array, $value_array, $map['CORE'], $selected_type, $get_mode, $line_number, $check, $hash_key, $map['WPML'], $map['ACF'], $map['TYPES'], $update_based_on);
			}
		}
		foreach ($map as $group_name => $group_value) {
			switch ($group_name) {
